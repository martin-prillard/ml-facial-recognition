...
if __name__ == "__main__":
    ...
    y_train = train_facile['label'].flatten()
    X_train = train_facile['X']

    model = Oasis(n_iter=800000, # n_iter=100000, il faudrait 8 000 000
                  do_psd=True, 
                  psd_every=5000, # psd_every apres x iterations, il refait la projection sur le cone, il faudrait 50 000
                  save_every=100000, # save training model every x iterations
                  save_path="tmp/oasis_test")

    model = model.fit(X_train,y_train,verbose=True)

    W = model._weights.view()
    # save the oasis matrix
    np.savetxt('tmp/oasis_matrix.txt', W)
