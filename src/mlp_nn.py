...
def generate_pairs(label, n_pairs, positive_ratio, replace=False, random_state=42):
    ...
    for i in range(n_pairs):
        if rand_vec[i] <= positive_ratio:
            ...
            # Try to create positive pairs with different ids
            if replace:
                if len(idx_same)!=1:
                    idx_same=list(idx_same)
                    idx_same.remove(pairs_idx[i, 0])
                    idx_same=np.array(idx_same)
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=10000,
    ...
    """
    print 'load dataset mat'
    dataset = io.loadmat('data/data_train_facile.mat')

    # train test splitting
    X_train, X_test, y_train, y_test = train_test_split(normalize(dataset['X']), #normalizing data
                                                        dataset['label'],
                                                        test_size=0.20) 
    # VAL
    pairs_val_idx, y_val = generate_pairs(labels, 
                                          10000, 
                                          0.5, 
                                          replace=False, 
                                          random_state=45)
    X_val = np.concatenate((X[pairs_val_idx[:,0],:],
                            X[pairs_val_idx[:,1],:]),axis=1)
